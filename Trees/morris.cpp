...
void tree::morris_inorder()
{
    node * curr = root;
    cout<<"\nMorris------\n";
    while(curr)
    {
        if(curr->left)//if left subtree
        {
            //find its rightest
            node * left_st = curr->left;

            while(true)
            {
                if(left_st->right == NULL)
                {
                    left_st->right = curr; 
                    curr = curr->left;
                    break;
                }
                else if(left_st->right == curr)//left st complete
                {
                    cout<<"\nData ; "<<curr->data;
                    left_st->right = NULL;
                    curr = curr->right;
