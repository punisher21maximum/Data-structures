...
public:
    ...
    int search_root(int arr[], int x, int n);
    void printPostOrder(int inA[], int preA[], int n);
...
int tree::search_root(int arr[], int x, int n)
{
    for(int i=0; i<n; i++)
    {
        if(arr[i] == x) return i;
...
void tree::printPostOrder(int inA[], int preA[], int n)
{
    int root = search_root(inA, preA[0], n);

    if(root!=0) printPostOrder(inA, preA+1, root);

    if(root!=n-1) printPostOrder(inA+root+1, preA+root+1, n-root-1);
